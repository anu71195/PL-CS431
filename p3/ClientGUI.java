...
//        String name = (String) v.elementAt(0);
    ...
    private void refreshAvls() {
        DefaultTableModel model =  (DefaultTableModel)item_contents.getModel();
        sendToServer("availability");
        String reply=getReply();
        String avls[] = reply.split(",");
        // expected 4 avls value
        if(avls.length!=4){
            JOptionPane.showMessageDialog(null,"ERROR: Invalid availability data!");
            return;
        }
        int quant,price;
        for (int i=0; i<model.getRowCount(); i++)
        {
            price=Integer.parseInt((String)model.getValueAt(i,2));
            quant=Integer.parseInt((String)model.getValueAt(i, 1));
            model.setValueAt(quant*price,i,3);
            model.setValueAt((avls[i].equals("-1")?"inf":avls[i]),i,4);
        }
    }
